...
class BookingCooldownException implements Exception {
  final DateTime availableAt;

  const BookingCooldownException(this.availableAt);
}
...
class _ConsultationBookingPageState
  ...
  DateTime? _bookingCooldownUntil;
  ...
  bool get _bookingCooldownActive {
    final until = _bookingCooldownUntil;
    return until != null && until.isAfter(DateTime.now());
  }

  String get _bookingCooldownLabel {
    final until = _bookingCooldownUntil;
  ...
  bool get _canConfirmBooking {
    if (_booking || _bookingCooldownActive) {
      return false;
    }
  ...
  void _startCooldownTicker() {
    _cooldownTimer = Timer.periodic(
      const Duration(seconds: 30),
        ...
        if (_bookingCooldownUntil != null) {
          setState(() {
            if (!_bookingCooldownActive) {
              _bookingCooldownUntil = null;
            }
          });
        }
      },
  ...
  Future<void> _loadBookingData() async {
    ...
    try {
      ...
      setState(() {
        ...
        _reports = reports;
        _bookingCooldownUntil = _readSubscriptionDate(
          userData['nextBookingAllowedAt'],
        );
        _loading = false;
      });
    } catch (_) {
  ...
  Future<DateTime?> _readLatestBookingCooldown(
      String userId,
      ) async {
    ...
    return _readSubscriptionDate(
      data['nextBookingAllowedAt'],
    );
  }
  ...
  Future<void> _confirmBooking() async {
    ...
    try {
      final latestCooldown = await _readLatestBookingCooldown(user.uid);

      if (latestCooldown != null &&
          latestCooldown.isAfter(DateTime.now())) {
        if (mounted) {
          setState(() {
            _bookingCooldownUntil = latestCooldown;
          });
        }

        _toast(
          'You recently booked an appointment. Please wait $_bookingCooldownLabel before booking again.',
        );
        return;
      }
    } catch (_) {
      // If the cooldown check cannot be loaded, the Firestore transaction
      // below still performs the authoritative check before creating a booking.
    }
    ...
    try {
      ...
      userReference.collection('appointments').doc();

      final now = DateTime.now();
      final nextBookingAllowedAt =
      ...
      await FirebaseFirestore.instance.runTransaction(
          ...
          final currentCooldown = _readSubscriptionDate(
            userData['nextBookingAllowedAt'],
          );

          if (currentCooldown != null &&
              currentCooldown.isAfter(DateTime.now())) {
            throw BookingCooldownException(currentCooldown);
          }
          ...
          transaction.set(
            userReference,
            {
              'nextBookingAllowedAt':
              Timestamp.fromDate(nextBookingAllowedAt),
            },
      ...
      if (mounted) {
        setState(() {
          _bookingCooldownUntil = nextBookingAllowedAt;
        });
      }
    ...
    } on BookingCooldownException catch (e) {
      if (mounted) {
        setState(() {
          _bookingCooldownUntil = e.availableAt;
        });
      }
          ...
          child: Column(
            ...
            children: [
              ...
              if (_bookingCooldownActive) ...[
                ...
                _BookingCooldownCard(
                  remainingText: _bookingCooldownLabel,
                ),
              ],
          ...
          children: [
            ...
            SizedBox(
              ...
              FilledButton.icon(
                ...
                label:
                Text(
                  _bookingCooldownActive
                      ? 'Book again in $_bookingCooldownLabel'
...
class _BookingCooldownCard extends StatelessWidget {
  final String remainingText;

  const _BookingCooldownCard({
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            child: Text(
              'You recently booked an appointment. You can book another in $remainingText.',
