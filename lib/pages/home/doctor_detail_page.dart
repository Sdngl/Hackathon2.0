  ...
  Future<void> _callDoctor(BuildContext context) async {
    ...
    if (phone == null || phone.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('This doctor does not have a phone number listed.'),
        ),
      );
      return;
    }
...
class _DoctorImage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Supports normal URLs, Base64 data URLs, and raw Base64 strings.
    if (_looksLikeBase64Image(value)) {
      try {
        final bytes = base64Decode(_cleanBase64(value));
  ...
  bool _looksLikeBase64Image(String value) {
    ...
    return value.length > 100 && !value.contains(' ');
  }
