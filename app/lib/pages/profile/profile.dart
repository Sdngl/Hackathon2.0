...
import 'appoinment_list_page.dart';
...
class _ProfileState extends State<Profile>
  ...
  Future<void> _loadProfile() async {
    ...
    try {
      ...
      final appointments =
      values[3] as QuerySnapshot<Map<String, dynamic>>;

      final appointmentList = appointments.docs
          .map(
            (doc) => <String, dynamic>{
          'id': doc.id,
          ...doc.data(),
      ...
      appointmentList.sort((a, b) {
        final aDate = _date(a['createdAt']) ??
            _date(a['appointmentDate']) ??
            DateTime.fromMillisecondsSinceEpoch(0);
        final bDate = _date(b['createdAt']) ??
            _date(b['appointmentDate']) ??
            DateTime.fromMillisecondsSinceEpoch(0);
      ...
      Navigator.of(context).push(
        MaterialPageRoute(
          builder: (_) => AppointmentDetailPage(
            appointment: _appointments.first,
          ),
        ),
      );
      return;
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const _SectionTitle(
                    'Appointments',
                    subtitle: 'Your booked consultations',
                  ),
                  const SizedBox(height: 10),
                  if (_appointments.isEmpty)
                    ...
                    if (_appointments.length > 1) ...[
                      ...
                      Align(
                        ...
                        child: TextButton.icon(
                          ...
                          label: Text(
                            'View all ${_appointments.length} appointments',
                          ),
...
class _AppointmentCard extends StatelessWidget {
  ...
  String _statusLabel() {
    final raw = _read('status', fallback: 'confirmed').toLowerCase();
  ...
  Color _statusColor() {
    ...
    if (status == 'confirmed') return const Color(0xFF11786D);
    if (status == 'completed') return const Color(0xFF3B74E0);
    if (status == 'cancelled') return const Color(0xFFD92D20);
  ...
  Color _statusBackground() {
    ...
    if (status == 'confirmed') return const Color(0xFFE5F7F1);
    if (status == 'completed') return const Color(0xFFEAF1FF);
    if (status == 'cancelled') return const Color(0xFFFFECEA);
    return const Color(0xFFFFF5D6);
  }

  String _consultationLabel() {
    final type = _read('consultationType', fallback: 'clinic').toLowerCase();

    if (type == 'video') return 'Video Consultation';
    if (type == 'clinic' || type == 'in_person' || type == 'in-person') {
      return 'Clinic Consultation';
    }

    return _read('consultationType', fallback: 'Consultation');
  }

  @override
  Widget build(BuildContext context) {
    final doctorName = _read('doctorName', fallback: 'Doctor');
    final specialization =
    _read('doctorSpecialization', fallback: 'Specialist');
    final consultationType =
    _read('consultationType', fallback: 'clinic').toLowerCase();
    final appointmentDate = _date(appointment['appointmentDate']);
    final appointmentTime = _read('appointmentTime');
    final clinicName = _read('clinicName');
    final clinicAddress = _read('clinicAddress');
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ...
              Expanded(
                child: Text(
                  !isVideo && clinicName.isNotEmpty
                      ? 'Clinic Consultation • $clinicName'
                      : _consultationLabel(),
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
                  style: const TextStyle(
          ...
          if (!isVideo && clinicAddress.isNotEmpty) ...[
            const SizedBox(height: 8),
            Row(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
